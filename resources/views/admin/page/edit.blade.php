...
<div class="box box-primary">
  ...
  <form role="form" method="post"
    action=" @if(isset($page->id)) {{ route('page.update',$page->id) }} @else {{ route('page.store') }} @endif ">
    {!! csrf_field() !!}
    @if(isset($page->id))
    ...
    <div class="box-body">
      <div class="form-group">
        ...
        <input class="form-control" placeholder="Inserir Nome" type="text" name="title"
          value="@if(isset($page)){{$page->title}}@endif">
      </div>
      <div class="form-group">
        <label>Descrição</label>
        <input class="form-control" name="description" type="text"
          value="@if(isset($page)){{$page->description}}@endif">
      </div>
      <div class="form-group">
        <label>Conteudo</label>
        <textarea class="form-control editor" rows="5"
          name="content">@if(isset($page)){{$page->content}}@endif</textarea>
      </div>
      <div class="form-group">
        <label>Url</label>
      ...
      <div class="form-group">
        ...
        <input class="form-control" name="meta_keywords" type="text"
          value="@if(isset($page)){{$page->meta_keywords}}@endif">
      </div>
      <div class="form-group">
        <label>Meta Description</label>
        <input class="form-control" name="meta_description" type="text"
          value="@if(isset($page)){{$page->meta_description}}@endif">
      </div>
    </div>
